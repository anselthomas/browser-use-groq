...
class Agent:
	...
	def set_tool_calling_method(self, tool_calling_method: Optional[str]) -> Optional[str]:
		if tool_calling_method == 'auto':
			...
			elif self.chat_model_library == 'AzureChatGroq':
				return 'function_calling'
			else:
				return None
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		# 🔥 **Filter out image messages before sending to Groq**
		filtered_messages = []
		for msg in input_messages:
			if isinstance(msg.content, list):
				text_only_content = [item for item in msg.content if isinstance(item, dict) and item.get('type') == 'text']
				if text_only_content:
					filtered_messages.append(SystemMessage(content=text_only_content))
			elif isinstance(msg.content, str):
				filtered_messages.append(msg)

		# 🔥 **Log if images were removed**
		logger.info(f"Filtered input messages: {len(filtered_messages)} out of {len(input_messages)}")
		...
		response: dict[str, Any] = await structured_llm.ainvoke(filtered_messages)  # type: ignore

		parsed: AgentOutput | None = response['parsed']
		if parsed is None:
			raise ValueError('Could not parse response.')

		# Limit the number of actions to prevent excessive calls
